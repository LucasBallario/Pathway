...
export const searchPublicRecords = async (formData) => { 
  ...
  console.log(" [DEBUG] Iniciando Public Records Engine (Modo LITE)...");
  
  const name = formData.name;
  const rawRegion = formData.region ? formData.region.trim() : "";
  
  let regionTextForQuery = "";
  let regionParamForDDG = "us-en"; 

  const regionMap = {
      'us': 'us-en', 'uk': 'uk-en', 'fr': 'fr-fr', 'es': 'es-es',
      'ar': 'ar-es', 'mx': 'mx-es', 'br': 'br-pt', 'co': 'co-es', 
      'cl': 'cl-es', 'pe': 'pe-es', 've': 've-es', 'ec': 'ec-es', 
      'uy': 'uy-es', 'py': 'py-es', 'bo': 'bo-es', 'cr': 'cr-es', 
      'pa': 'pa-es', 'gt': 'gt-es', 'hn': 'hn-es', 'sv': 'sv-es', 
      'ni': 'ni-es', 'do': 'do-es', 'cu': 'cu-es', 'pr': 'pr-es'  
  };

  if (rawRegion.length === 2 && regionMap[rawRegion.toLowerCase()]) {
      regionParamForDDG = regionMap[rawRegion.toLowerCase()];
      console.log(` [DEBUG] Región detectada: ${regionParamForDDG}`);
  } else if (rawRegion.length > 2) {
      regionTextForQuery = `"${rawRegion}"`;
      console.log(` [DEBUG] Región texto: ${regionTextForQuery}`);
  }

  const corporateTerms = [
      "LLC", "Inc", "Corp", "Ltd", "S.A.", "S.R.L.", "SAS", 
      "Director", "Founder", "Socio", "Gerente", "Dueño",
      "CUIT", "RUT", "RFC", "Tax ID"
  ];
  ...
  const fileTypes = ["pdf", "doc", "docx", "xls", "xlsx", "csv"];

  const queries = [];
  queries.push(`"${name}" ${regionTextForQuery} (${corporateTerms.join(' OR ')})`);
  queries.push(`"${name}" ${regionTextForQuery} (site:gov.* OR site:gob.* OR ${governmentTerms.join(' OR ')})`);
  queries.push(`"${name}" ${regionTextForQuery} (${fileTypes.map(f=>`filetype:${f}`).join(' OR ')})`);

  const getPublicRecordsLite = async (query, regionCode) => {
      try {
          ...
          console.log(` [DEBUG] POST a LITE: ${query.substring(0, 40)}...`);
          ...
          const response = await fetch(targetUrl, {
              ...
              headers: {
                  ...
                  "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36", // User agent móvil
          ...
          if (!response.ok) {
              console.error(` [DEBUG] Error Status: ${response.status}`);
              return "";
          }
          
          const text = await response.text();
          if (text.length < 500) console.log("⚠️ [ALERTA] Respuesta sospechosamente corta.");
  ...
  const parsePublicRecordsLite = (html, originalQuery) => {
    if (!html) return [];
    const $ = cheerio.load(html);
    const results = [];
    ...
    // Palabras clave de navegación interna de DDG Lite que queremos ignorar
    const internalNav = ['next >', '< prev', 'settings', 'privacy', 'terms', 'feedback', 'region'];

    const allLinks = $('a');
    console.log(`   🔎 [DEBUG] Cheerio ve ${allLinks.length} enlaces TOTALES en el HTML.`);
    ...
    allLinks.each((index, element) => {
        ...
        // 1. Filtrado básico de nulidad
        if (!url) return;

        // 2. DEBUG VISUAL: Ver qué está pasando por el parser
        // (Imprime solo los primeros 5 para no saturar)
        if (index < 5) console.log(`      Link detectado: ${url} (Texto: ${title})`);

        // 3. Filtrado de Navegación Interna
        if (url.startsWith('/lite/')) return; // Paginación interna
        if (url.startsWith('?q=')) return; // Búsquedas relacionadas
        if (internalNav.some(nav => title.toLowerCase().includes(nav))) return;

        // 4. Filtrado de Redes Sociales
        if (forbiddenWords.some(w => url.toLowerCase().includes(w))) return;

        // 5. Validación de URL externa
        // En Lite, los resultados suelen ser directos (http...)
        if (url.startsWith('http')) {
            
            // Etiquetas visuales
            if (url.endsWith('.pdf')) title = `[PDF] ${title}`;
            if (url.includes('.gov') || url.includes('.gob')) title = `[OFICIAL] ${title}`;

            results.push({
                title: title || url,
                snippet: `Registro (Lite): ${originalQuery}`,
                url: url,
  ...
  for (const q of queries) {
      const html = await getPublicRecordsLite(q, regionParamForDDG); 
      const found = parsePublicRecordsLite(html, q);

      if (found.length > 0) {
          console.log(`    Encontrados ${found.length} registros.`);
          allRecords.push(...found);
      }

      // Pausa  para evitar bloqueo en Lite
      await new Promise(r => setTimeout(r, 3000));
  }

  const uniqueRecords = allRecords.filter((v,i,a)=>a.findIndex(t=>(t.url===v.url))===i);
  console.log(` [FIN] Total Únicos: ${uniqueRecords.length}`);
