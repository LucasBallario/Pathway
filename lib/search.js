...
export const searchGoogle = async (data) => { 
  ...
  const buildGoogleUrl = (query, region) => {
    // CAMBIO CLAVE: Usamos DuckDuckGo HTML para evitar bloqueos
    const baseUrl = 'https://html.duckduckgo.com/html/';
    
    // Mapeo básico de regiones (ej: 'US' -> 'us-en')
  ...
  const parseGoogleResults = (html) => {
    ...
    // Buscamos TODOS los links del cuerpo
    $('a').each((index, element) => {
      ...
      // --- PASO 1: DETECTAR Y LIMPIAR REDIRECCIONES DE DDG ---
      // Los links suelen venir así: //duckduckgo.com/l/?uddg=https%3A%2F%2Fsitio.com...
      if (url.includes('uddg=')) {
          try {
            // Extraemos lo que está después de "uddg=" y antes del siguiente "&"
            url = decodeURIComponent(url.split('uddg=')[1].split('&')[0]);
          } catch (e) {
            // Si falla la decodificación, ignoramos este link
            return;
          }
      }
      ...
      if (title && url.startsWith('http')) {
        // Filtramos basura interna
        if (!url.includes('duckduckgo.com') && 
            !url.includes('google.com') && 
            !url.includes('search_plus_one')) {
           
           // Filtro de calidad: Títulos muy cortos suelen ser botones de "Atrás" o "Siguiente"
           if (title.length > 5) {
               results.push({
                 title: title,
