...
export const searchPublicRecords = async (formData) => { 
  ...
  const parsePublicRecords = (html, originalQuery) => {
    ...
    const $ = cheerio.load(html);
    const results = [];
    const forbiddenWords = ['facebook', 'instagram', 'twitter', 'linkedin', 'tiktok', 'pinterest', 'youtube', 'duckduckgo', 'google'];

    const totalLinks = $('a').length;
    console.log(`   🔎 [DEBUG] Cheerio ve ${totalLinks} enlaces en el HTML.`);
    ...
    $('a').each((index, element) => {
        ...
        //  Si tiene 'uddg=' es un resultado externo
        if (rawHref.includes('uddg=')) {
            try {
                // Decodifica la URL real
                // Formato típico: /l/?uddg=https%3A%2F%2Fejemplo.com&rut=...
                const realUrl = decodeURIComponent(rawHref.split('uddg=')[1].split('&')[0]);

                if (forbiddenWords.some(w => realUrl.toLowerCase().includes(w))) return;

                if (realUrl.endsWith('.pdf')) title = `[PDF] ${title}`;
                if (realUrl.includes('.gov') || realUrl.includes('.gob')) title = `[OFICIAL] ${title}`;
                
                if (!title || title.length < 3) title = realUrl;

                results.push({
                    title: title,
                    snippet: `Registro posible relacionado con: ${originalQuery}`,
                    url: realUrl,
