...
// Lista de instancias para que rote si una falla
const SEARX_INSTANCES = [
  "https://searx.be",
  "https://searx.space",
  "https://searx.ngn.tf",
  "https://searx.fmac.xyz",
  "https://north-america.searx.ngn.tf"
];

export const searchPublicRecords = async (formData) => { 

console.log(" [DEBUG] Iniciando Public Records Engine (Motor: SearXNG JSON)...");

const name = formData.name;
const rawRegion = formData.region ? formData.region.trim() : "";

let regionTextForQuery = "";
let regionParamForEngine = "us-en"; 
...
if (rawRegion.length === 2 && regionMap[rawRegion.toLowerCase()]) {
    regionParamForEngine = regionMap[rawRegion.toLowerCase()];
    console.log(` [DEBUG] Región detectada: ${regionParamForEngine}`);
...
const fetchAndParseSearx = async (query, regionCode) => {
    // Intentamos rotar por las instancias hasta que una funcione
    for (const instance of SEARX_INSTANCES) {
        try {
            // Convertir  código de región (ej: ar-es) a idioma para SearXNG (ej: es)
            const lang = regionCode.split('-')[1] || 'all'; 

            // Construir URL pidiendo JSON
            const url = `${instance}/search?q=${encodeURIComponent(query)}&format=json&language=${lang}`;
            
            console.log(` [DEBUG] Conectando a instancia: ${instance}...`);

            // aborta si tardda  más de 6 segundos
            ...
            if (response.ok) {
                ...
                if (data.results && Array.isArray(data.results)) {
                    return data.results.map(item => ({
                        ...
                        snippet: item.content || `Registro encontrado para: ${name}`,
                        url: item.url,
            ...
            // Si falla esta instancia, el bucle sigue  a la siguiente
        }
    }
    return []; // Si todas fallan
};

const allRecords = [];

for (const q of queries) {
    console.log(` [DEBUG] Buscando: ${q.substring(0, 40)}...`);
    
    const found = await fetchAndParseSearx(q, regionParamForEngine);

    if (found.length > 0) {
        console.log(`    Encontrados ${found.length} registros.`);
        allRecords.push(...found);
    } else {
        console.log(`    Sin resultados en esta vuelta.`);
    }
