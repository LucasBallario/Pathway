...
export const searchPublicRecords = async (formData) => { 
  ...
  console.log(" [DEBUG] Iniciando Public Records Engine (Modo GET)...");
  
  const name = formData.name;
  const rawRegion = formData.region ? formData.region.trim() : "";
  
  let regionTextForQuery = "";
  let regionParamForDDG = ""; 
  ...
  if (rawRegion.length === 2 && regionMap[rawRegion.toLowerCase()]) {
      ...
      console.log(` [DEBUG] Región detectada por código: ${regionParamForDDG}`);
  } else if (rawRegion.length > 2) {
      regionTextForQuery = `"${rawRegion}"`;
      console.log(` [DEBUG] Región detectada por nombre: ${regionTextForQuery}`);
  }
  ...
  queries.push(`"${name}" ${regionTextForQuery} (${corporateGroup})`);
  queries.push(`"${name}" ${regionTextForQuery} (site:gov.* OR site:gob.* OR ${governmentGroup})`);
  queries.push(`"${name}" ${regionTextForQuery} (${fileTypeGroup})`);

  console.log(`📋 [DEBUG] Queries generadas: ${queries.length}`);

  const getPublicRecordsHtml = async (query, regionCode) => {
      try {
          if (!query) return "";
          ...
          if (regionCode) {
              ...
              params.append('kl', 'us-en'); // Default seguro
          }

          const targetUrl = `https://html.duckduckgo.com/html/?${params.toString()}`;
          console.log(` [DEBUG] Enviando GET a: ${targetUrl.substring(0, 60)}...`);
          ...
          if (text.includes("<title>DuckDuckGo</title>")) {
              console.log("⚠️ [ALERTA] Redirección a Home (Bloqueo Soft).");
          } else if (text.includes("No results")) {
              console.log("🔸 [INFO] Sin resultados.");
          } else {
              console.log(" [ÉXITO] Resultados recibidos.");
          }
  ...
  const parsePublicRecords = (html, originalQuery) => {
      if (!html) return [];
      const $ = cheerio.load(html);
      const results = [];
      const forbiddenWords = ['facebook', 'instagram', 'twitter','linkedin', 'tiktok', 'pinterest', 'youtube'];

      // El selector en la versión HTML suele ser .result__a o enlaces generales
      $('a.result__a, .result__body a').each((index, element) => {
          const link = $(element);
          let url = link.attr('href');
          ...
          // Decodificar URL de redirección de DDG
          if (url.includes('uddg=')) {
              try { url = decodeURIComponent(url.split('uddg=')[1].split('&')[0]); } catch (e) {}
          }
          
          if (forbiddenWords.some(word => url.toLowerCase().includes(word))) return;
          if (url.includes('google.com') || url.includes('duckduckgo.com')) return;

          if (url.endsWith('.pdf') || url.endsWith('.xls') || url.endsWith('.doc')) title = `[DOC] ${title}`;
          if (url.includes('.gov') || url.includes('.gob')) title = `[OFICIAL] ${title}`;

          if (title && url.startsWith('http')) {
              results.push({
                  title: title,
                  snippet: `Registro encontrado para: ${originalQuery}`,
                  url: url,
      ...
      // Fallback: Si la estructura cambió, intentar búsqueda genérica de enlaces
      if (results.length === 0) {
           $('a').each((index, element) => {
                const link = $(element);
                let url = link.attr('href');
                ...
                if (url && url.includes('uddg=')) {
                     try { url = decodeURIComponent(url.split('uddg=')[1].split('&')[0]); } catch (e) {}
                     if (!forbiddenWords.some(w => url.toLowerCase().includes(w))) {
                         results.push({ title, snippet: `Registro (Fallback): ${originalQuery}`, url, source: 'public_record' });
                     }
                }
           });
      }
  ...
  for (const q of queries) {
      const html = await getPublicRecordsHtml(q, regionParamForDDG); 
      const found = parsePublicRecords(html, q);
