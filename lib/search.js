...
export const searchPublicRecords = async (formData) => { 
  ...
  console.log(" [DEBUG] Iniciando Public Records Engine...");
  ...
  const regionMap = {
    'us': 'us-en', // Estados Unidos
      'uk': 'uk-en', // Reino Unido
      'fr': 'fr-fr', // Francia
      'es': 'es-es', // España
      ...
      'ni': 'ni-es', // Nicaragua
      'do': 'do-es', // República Dominicana
      'cu': 'cu-es', // Cuba
      'pr': 'pr-es'
  }

  if (rawRegion.length === 2 && regionMap[rawRegion.toLowerCase()]) {
    // si es un código (ej: "ar")
    // No lo ponemos en el texto (ensucia), pero lo usamos en el motor
    ...
    // si es un nombre (ej: "Florida", "Argentina")
    // Lo ponemos en el texto entre comillas para filtrar
  ...
  const buildPublicRecordsURL = (query) => {
      return query
  };

  const getPublicRecordsHtml = async (query,regionCode) => {
    try {
      console.log(`🌐 [DEBUG] Enviando POST a DuckDuckGo: ${query.substring(0, 50)}...`);
      if (!query) return "";
      const params = new URLSearchParams();
      params.append('q', query);
      params.append('b', ''); // Parámetro opcional de DDG

      params.append('kl', regionCode || 'us-en');
      ...
      if (!response.ok) {
          console.error(` [DEBUG] Fetch Error Status: ${response.status}`);
          return "";
      }

      const text = await response.text();
      const titleMatch = text.match(/<title>(.*?)<\/title>/);
      const pageTitle = titleMatch ? titleMatch[1] : "Sin título";
      console.log(` [DEBUG] HTML recibido. Longitud: ${text.length} caracteres`);
      
      if (text.includes("<title>DuckDuckGo</title>")) {
         console.log("[ALERTA] Seguimos recibiendo la Home. DuckDuckGo está difícil hoy.");
      } else {
         console.log(" [ÉXITO] Parece una página de resultados real.");
      }
  ...
  const parsePublicRecords = (html, originalQuery) => {
      ...
      console.log(`🔎 [DEBUG] Cheerio encontró ${totalLinks} enlaces en el HTML.`);
      ...
      $('a').each((index, element) => {
          ...
          if (index < 3) console.log(`   [Analizando]: ${url}`);
          ...
          if (title && url.startsWith('http')) {
              ...
              results.push({
                  title: title,
                  snippet: `Public record found via: ${originalQuery}`,
                  url: url,
  ...
  for (const q of queries) {
    ...
    if (found.length > 0) {
        console.log(`    Encontrados ${found.length} registros en esta vuelta.`);
  ...
  console.log(`🏁 [DEBUG] FINAL: Se encontraron ${uniqueRecords.length} registros únicos.`);
