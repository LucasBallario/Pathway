...
export const searchPublicRecords = async (formData) => { 
  ...
  console.log(" [DEBUG] Iniciando Public Records Engine (Motor: Serper.dev Google)...");

  if (!SERPER_API_KEY || SERPER_API_KEY.includes("TU_CLAVE")) {
      console.error(" [ERROR] FALTA LA API KEY. Regístrate en serper.dev y pégala en el código.");
      return [];
  }
  ...
  if (rawRegion && rawRegion.length >= 2) {
      regionText = `"${rawRegion}"`;
  }

  const corporateTerms = [
    "LLC", "Inc", "Corp", "Ltd", "PLC", 
    "S.A.", "S.R.L.", "S.A.S.", "S.C.", "SpA", 
    "Ltda", "EIRELI", "MEI", 
    "GmbH", "AG", "SARL", "SAS", "S.p.A.", 
    
    "Director", "Founder", "CEO", "CFO", "Owner", "Partner", 
    "Socio", "Gerente", "Dueño", "Presidente", "Accionista", 
    "Sócio", "Proprietário", "Acionista", 
    
    "CUIT", "CUIL", "RUT", "RFC", "DNI", "NIE", "CIF", "NIF", 
    "CNPJ", "CPF" 
];

const governmentTerms = [
    "Boletin Oficial", "Official Gazette", "Diario Oficial", "Federal Register",
    "Diário da República", "Amtsblatt", "Journal Officiel",
    
    "Sentencia", "Judgment", "Verdict", "Sentença", "Urteil",
    "Juzgado", "Court", "Tribunal", "Fiscál", 
    "Docket", "Expediente", "Lawsuit", "Demanda", "Processo",
    
    "Multa", "Fine", "Sanction", "Sancion", "Sanción",
    "Deuda", "Debt", "Dívida", "Mora",
    "Quiebra", "Bankruptcy", "Falência", "Insolvency",
    "Licitacion", "Tender", "Concurso", "Bidding",
    "Wanted", "Buscado", "Antecedentes"
];

const fileTypes = ["pdf", "doc", "docx", "xls", "xlsx", "csv", "txt"];
  
const queries = [];
  

const topCorporate = corporateTerms.slice(0, 25).join(' OR ');
queries.push(`"${name}" ${regionText} (${topCorporate})`);

const topGovernment = governmentTerms.slice(0, 25).join(' OR ');
queries.push(`"${name}" ${regionText} (site:gov.* OR site:gob.* OR ${topGovernment})`);

queries.push(`"${name}" ${regionText} (${fileTypes.map(f=>`filetype:${f}`).join(' OR ')})`);


const taxIds = ["CUIT", "RUT", "RFC", "CNPJ", "Tax ID", "VAT", "NIF"].join(' OR ');
queries.push(`"${name}" ${regionText} (${taxIds})`);
  ...
  const searchSerper = async (query) => {
      try {
          ...
          const raw = JSON.stringify({
              "q": query,
              "num": 10 // Trae 10 resultados por query
          });
          ...
          console.log(` [Serper] Buscando: ${query.substring(0, 40)}...`);
          ...
          // Serper devuelve los resultados en la propiedad 'organic'
          if (result.organic && result.organic.length > 0) {
              console.log(`    [Serper] Encontrados: ${result.organic.length}`);
              ...
              console.log(`    [Serper] 0 resultados.`);
              return [];
          }

      } catch (error) {
          console.error(` [Serper] Excepción:`, error);
          return [];
      }
  };

  for (const q of queries) {
      const found = await searchSerper(q);
