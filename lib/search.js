...
export const searchPublicRecords = async (formData) => { 
  ...
  const getPublicRecordsHtml = async (query) => {
    try {
      ...
      params.append('kl', 'us-en'); // Forzar región (opcional, ayuda a estandarizar)
      ...
      if (text.includes("<title>DuckDuckGo</title>")) {
         console.log("⚠️ [ALERTA] Seguimos recibiendo la Home. DuckDuckGo está difícil hoy.");
      } else {
  ...
  for (const q of queries) {
    const html = await getPublicRecordsHtml(q);
    ...
    if (found.length > 0) {
        ...
        console.log(`   🤷‍♂️ Sin resultados relevantes.`);
    }
