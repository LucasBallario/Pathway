...
export const searchPublicRecords = async (formData) => { 
  ...
  const getPublicRecordsHtml = async (url) => {
      try {
          ...
          // Si sigue fallando imprime las primeras líneas de lo que DDG dice
          if (text.length < 20000) {
            console.log("⚠️ [ALERTA] HTML sospechosamente corto. Contenido inicial:");
            console.log(text.substring(0, 500)); // Imprime los primeros 500 caracteres
        }
          
          if (text.includes("If this error persists, please let us know")) {
              console.error("⛔ [DEBUG] BLOQUEO DETECTADO: DuckDuckGo nos dio error 403/Rate Limit.");
          }
  ...
  const parsePublicRecords = (html, originalQuery) => {
      ...
      // DEBUG: Ver cuántos enlaces hay en total antes de filtrar
      ...
      $('a').each((index, element) => {
          ...
          // DEBUG: Ver qué enlaces está analizando (solo los primeros 3 para no llenar la consola)
          if (index < 3) console.log(`   🔗 [Analizando]: ${url}`);

          // Filtros
          if (forbiddenWords.some(word => url.toLowerCase().includes(word))) {
              // console.log(`   🗑️ [Descartado - Social]: ${url}`);
              return;
          }
  ...
  for (const q of queries) {
      const url = buildPublicRecordsURL(q);
      const html = await getPublicRecordsHtml(url);
      const found = parsePublicRecords(html, q);

      if (found.length > 0) allRecords.push(...found);
